...
module RAFL
  ...
  def self.calc_rms(audio_samples, format)
    #squaresum = 0
  ...
  def self.calc_peak(audio_samples, format)
    calc_dbfs(audio_samples.max.to_f, format.bit_depth)
  end

  def self.calc_dbfs(sample_value, bit_depth)
    range = (2 ** bit_depth) / 2
    (20*Math.log10(sample_value.to_f / range)).round_to(2)
  end

  def self.calc_sample_value(dbfs_value, bit_depth)
    range = (2 ** bit_depth / 2)
    (range * Math::E ** (1/20.0 * dbfs_value * (Math.log(2) + Math.log(5)))) - 1
  end

  def self.generate_white_noise(length_secs, peak_db, sample_rate, bit_depth)
  ...
  def self.generate_pink_noise(length_secs, peak_db, sample_rate, bit_depth)
    num_samples = (length_secs * sample_rate).to_i
    peak_samples = calc_sample_value(peak_db, bit_depth)
    output = []
  ...
  def self.generate_sine_wave(length_secs, peak_db, freq, sample_rate, bit_depth) #defective...drifts over time - rounding error?
    peak_samples = calc_sample_value(peak_db, bit_depth)
    output = []
    period = 1.0 / freq
